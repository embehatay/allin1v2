...
class FenPlayer(xbmc_player):
    ...
    def onPlayBackStarted(self) -> None:
        ...
        if self.subs_action == '1':
            ...
            try:
                Thread(target=Subtitles().get, args=(self, self.title, self.tmdb_id,
                       self.season or None, self.episode or None, self.url, "onPlayBackStarted")).start()
            except Exception:
                logger("Exception khi run subtile: " +
                       str(Exception), "on playback started")
                pass
            ...
            while self.isPlayingVideo():
                try:
                    ...
                    if xbmcgui.getCurrentWindowDialogId() == 10153:
                        logger("hien dialog subtitles: ", str(
                    ...
                    if self.curr_time - pass_time > 10:
                        ...
                        self.media_watched_marker()
                    if not self.subs_searched and self.subs_action != '2':
                        self.run_subtitles("isPlayingVideo")
                except:
                    pass
            hide_busy_dialog()
            if not self.media_marked:
    ...
    def run_subtitles(self, source):
        ...
        try:
            Thread(target=Subtitles().get, args=(self, self.title, self.tmdb_id, self.season or None,
                   self.episode or None, self.url, source, self.subs_action)).start()
        except:
            pass
    ...
    def info_next_ep(self):
        ...
        try:
            ...
            self.nextep_settings = {'use_window': use_window, 'window_time': window_time,
                                    'default_action': default_action, 'play_type': 'autoplay_nextep'}
        except:
            pass
    ...
    def get(self, fenPlayer, query, tmdb_id, season, episode, url, source, subs_action=None, secondary_search=False):
        ...
        def _downloaded_subs():
            ...
            if len(files) > 0:
                ...
                for item in files:
                    logger("So sanh de tim ra file sub: " +
                           search_filename, item)
        ...
        def _searched_subs():
            ...
            if self.subs_action == '1':
                ...
                dialog_list = ['[B]%s[/B] | [I]%s[/I]' %
                               (self.language.upper(), i['attributes']['release']) for i in os_results]
                sd_dialog_list = ['[B]%s[/B] | [I]%s[/I]' %
                                  (self.language.upper(), i['release_name']) for i in sd_results]
                dialog_list.extend(sd_dialog_list)
                ...
                self.pause()
                if chosen_sub == None or chosen_sub == []:
                    return False
                if (chosen_sub >= len(os_results)):
                    chosen_source = 'sd'
            else:
        ...
        subtitle_path = translate_path(
            ku.jsonrpc_get_system_setting("subtitles.custompath"))
        # logger("Đường dẫn subtile custom: ", subtitle_path)
        sub_filename = 'FENSubs_%s_%s_%s' % (
            tmdb_id, season, episode) if season else 'FENSubs_%s' % tmdb_id
